...
const travelSchema = z.object({
  ...
  budget: z.coerce
    .number({ message: "Introduce un presupuesto válido." })
    .min(1, "El presupuesto debe ser mayor a 0."),
});

type TravelFormInput = z.input<typeof travelSchema>;
type TravelFormOutput = z.output<typeof travelSchema>;
  ...
  return (
    <div className="rounded-xl border bg-card p-5 shadow-sm">
      <h3 className="font-semibold text-lg tracking-tight mb-1 flex items-center gap-2">
        <Plane className="h-4 w-4 text-primary" /> Planear Nuevo Viaje
      </h3>
      <p className="text-xs text-muted-foreground mb-4">
        Añade tu próximo destino al radar.
      </p>
